...
static void ScoreLineQIBSpecDialog_Reflesh(HWND hDlg)
{
	...
	ScoreLineQIBSpecDialog_RefleshUnit(listWnd, SWRSCHAR_MAX + 1, sumWinAll, sumLoseAll);
	...
	for (int h = 0; h < _countof(limits); ++h) {
		...
		ScoreLineQIBSpecDialog_RefleshUnit(listWnd, SWRSCHAR_MAX + 2 + h, sumWinAll, sumLoseAll);
	}
...
static LRESULT ScoreLineView_OnDoubleClick(HWND hParent, HWND hwnd)
{
	...
	if (ListView_SubItemHitTest(hwnd, &hti) != -1) {
		// キャラ部分のクリック
		if (hti.iItem >= 0 && hti.iItem < SWRSCHAR_MAX) {
			// 勝ちか負け
...
static BOOL ScoreLineQIBSpecDialog_InitListView(HWND hDlg)
{
	...
	LVPARAMFIELD(item.lParam).charId = SWRSCHAR_MAX;
	...
	for (int i = 1; i < SCORELINE_FILTER__MAX; i <<= 1) {
		switch(fltCustom.mask & i) {
			...
			case SCORELINE_FILTER__P2ID:
				title += 
					Formatter(_T("vs %s "), 
						g_characters[fltCustom.p2id].abbr);
				break;
			...
			case SCORELINE_FILTER__LIMIT:
				title += Formatter("最近%d戦", fltCustom.limit);
				break;
		}
	}
...
{
	ScoreLineQIBSpecDialog_InitParams *params = (ScoreLineQIBSpecDialog_InitParams*)lParam;

	if (params->pfilterDesc->mask & SCORELINE_FILTER__P1ID) {
		::SetProp(hDlg, PROP_PID, (HANDLE)params->pfilterDesc->p1id);
	} else if (params->pfilterDesc->mask & SCORELINE_FILTER__P2ID) {
		::SetProp(hDlg, PROP_PID, (HANDLE)params->pfilterDesc->p2id);
	}
	SetProp(hDlg, PROP_FILTER, (HANDLE)params->pfilterDesc);
	SetProp(hDlg, PROP_TRDIRLEFT, (HANDLE)params->trDirLeft);
