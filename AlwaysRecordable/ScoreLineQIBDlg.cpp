...
static void ScoreLineQIBDialog_Reflesh(HWND hDlg)
{
	...
	for (int h = 0; h < _countof(limits); ++h) {
		...
		ScoreLineQIBDialog_RefleshUnit(listWnd, SWRSCHAR_MAX + 2 + h, sumWinAll, sumLoseAll);
	}
...
static void ScoreLineQIBDialog_OnDestroy(HWND hDlg)
{
	...
	g_settings.WriteInteger(_T("QIBDlg.Display"), _T("X"), rct.left);
	g_settings.WriteInteger(_T("QIBDlg.Display"), _T("Y"), rct.top);
